...
### 5. matugen template keywords — mostly RESOLVED; end-to-end run still machine-side
Cross-checked the template's role names (`surface_container_high`, `outline_variant`, `surface_tint`, etc.) against matugen's real documentation — they check out on paper, more solid than originally feared. But this was never actually run end-to-end (a sandbox attempt to test it hit an unrelated toolchain issue). **Run `matugen color hex "#29D3F0" -v` as the very first test on the real machine, before testing anything downstream of it.**
**CORRECTED 2026-07-06:** the earlier claim that `.default` always resolves to the dark colour regardless of `-m` is **FALSE for matugen 4.1.0** (the shipped version) — verified from source: `main.rs:80` sets `default_scheme = args.mode` and `color.rs:127` resolves `.default` from `default_scheme`, so `.default` correctly follows `-m dark|light`. The light/dark toggle therefore produces a real change with the existing `.default` templates; no `.light`/`.dark` conversion is needed. Light mode's *cream* look is a separate post-matugen override (see the Theming section).
